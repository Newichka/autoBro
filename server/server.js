...
// --- Маршруты для пользователей (без изменений) ---
app.get("/users/:uid", (req, res) => {
  ...
  if (user) {
    ...
    res.status(404).json({ 
      ...
      user: { fullName: "", phone: "", country: "", city: "", address: "" } 
    }); 
  }
});
...
app.post("/users", (req, res) => {
  ...
  if (existingUser) {
    ...
    res.json({ message: "Профиль пользователя обновлен", user: db.get("users").find({ uid }).value() });
  } else {
    const newUser = { uid, ...userData, createdAt: new Date().toISOString() };
    db.get("users").push(newUser).write();
    res.status(201).json({ message: "Профиль пользователя создан", user: newUser });
  }
});
